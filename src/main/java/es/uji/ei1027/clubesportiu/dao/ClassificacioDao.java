...
public class ClassificacioDao {
    ...
    //
    //
    public List<Classificacio> getClassificacioPais(String nomPais) {
        try {
            return this.jdbcTemplate.query(
                    "SELECT * FROM classificacio WHERE nom_nadador=(SELECT Nom FROM Nadador WHERE Pais=?)",
                    new Object[] {nomPais}, new ClassificacioRowMapper());
        }
        catch(EmptyResultDataAccessException e) {
            return new ArrayList<Classificacio>();
        }
